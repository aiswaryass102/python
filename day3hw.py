...
# Check if credits are within valid range and not equal to minimum
credits_valid = (user_credits >= min_credits and user_credits <= max_credits) and (user_credits != min_credits)

# Bonus eligibility (subscribed OR credits greater than minimum)
bonus_eligible = is_subscribed or not (user_credits <= min_credits)

# Modify user_credits using operators
user_credits += 50   # add 50
user_credits -= 20   # subtract 20
user_credits *= 2    # multiply by 2
user_credits %= 150  # modulo 150

# Power calculation
power_result = user_credits ** 2

# Full access check (logged in AND subscribed)
